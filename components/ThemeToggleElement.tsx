...
export default function ThemeToggleElement() {
  ...
  return (
        ...
        >
            <i className={`fa-solid fa-sun text-2xl text-yellow-400 transition-all duration-300 ` + 
            ...
            <div className={'absolute h-8 w-8 z-10 rounded-full transition-all duration-500 z-10 ring-2  ' +
                (isDay? "translate-x-[160%] bg-[var(--primary-color)] ring-yellow-400" : "-translate-x-[25%] bg-gray-100 ring-purple-300" )}></div>
            <i className={"fa-solid fa-moon text-2xl text-purple-700 transition-all duration-300 " + 
                (isDay? "translate-x-[150%] translate-y-[150%] rotate-90 hidden" : "translate-x-2 translate-y-0 rotate-0" )}></i>
        </div>
  )
}
